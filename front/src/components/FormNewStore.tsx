...
const initialFormState : ThriftStore = {
  name: "",
  lng: 0,
  lat: 0,
...
export default function MyForm({addThriftStorePreview,className,}: {addThriftStorePreview?: (newStore: ThriftStore) => void;className?: string;}) {
  ...
  const handleAddClick = () => {
    setForm({
      ...form,
      openingHours: [...form.openingHours, { day: "", hours: "" }],
    });
  };
  ...
  const handleAddressSearch = async (event: { preventDefault: () => void }) => {
    ...
    if (data && data.length > 0) {
      ...
      if (addThriftStorePreview)
        addThriftStorePreview({
          ...form,
          ...
          return (
            <div key={i}>
              <input
                name="day"
                value={x.day}
                onChange={(e) => handleOpeningHoursChange(e, i)}
                placeholder="Day"
              />
              <input
                name="hours"
